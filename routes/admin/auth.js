...
router.post('/forgot_password', async (req, res) => {
    ...
    try {
        ...
        if (admin.is_blocked) return res.status(400).send({ status: "error", msg: "Account blocked" })
        if (admin.is_banned) return res.status(400).send({ status: "error", msg: "Account banned" })
        if (admin.is_deleted) return res.status(400).send({ status: "error", msg: "Account deleted" })
        ...
        const link = `${process.env.BASE_URL || 'http://localhost:7000'}/admin_auth/reset_password/${resetToken}`

        // Send email to admin email only
        await sendPasswordResetAdmin(admin.email, link);
    ...
    try {
      ...
      <html>
          ...
          <body>    
              ...
              <form action="/admin_auth/reset_password" method="post">
...
router.post("/reset_password", async (req, res) => {
    ...
    try {
        ...
        if (!admin) return res.status(400).send(`<h1>Password Reset</h1>No account found<p>`)
        if (admin.is_blocked) return res.status(400).send(`<h1>Password Reset</h1>Account blocked<p>`)
        if (admin.is_banned) return res.status(400).send(`<h1>Password Reset</h1>Account banned<p>`)
        if (admin.is_deleted) return res.status(400).send(`<h1>Password Reset</h1>Account deleted<p>`)
