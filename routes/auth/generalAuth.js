...
router.post('/forgot_password', async (req, res) => {
  ...
  try {
    ...
    */
    ...
    if (account.is_blocked) return res.status(400).send({ status: "error", msg: "Account blocked" })
    if (account.is_banned) return res.status(400).send({ status: "error", msg: "Account banned" })
    if (account.is_deleted) return res.status(400).send({ status: "error", msg: "Account deleted" })
    ...
    const link = `${process.env.BASE_URL || 'http://localhost:7000'}/auth_generalAuth/reset_password/${resetToken}`

    // Send appropriate email (or SMS later if implemented)
    if (type === 'user') {
      await sendPasswordReset(account.email, link)
    } else {
      await sendPasswordResetOrg(account.email, link)
    }
    ...
    return res.send(`<!DOCTYPE html>
      <html>
          ...
          <body>    
              ...
              <form action="/auth_generalAuth/reset_password" method="post">
...
router.post("/reset_password", async (req, res) => {
  ...
  try {
    ...
    // Check if account is active
    if (account.is_blocked) return res.status(400).send(`<h1>Password Reset</h1>Account blocked<p>`)
    if (account.is_banned) return res.status(400).send(`<h1>Password Reset</h1>Account banned<p>`)
    if (account.is_deleted) return res.status(400).send(`<h1>Password Reset</h1>Account deleted<p>`)
