...
// Get all conversations of the logged-in organization
router.post('/user_chats', authToken, async(req, res) => {
    const orgId = req.user._id

    try {
        const conversations = await Conversation.find({ participants: { $in: [orgId] } })
        .populate('participants', 'company_name email profile_img').sort({ last_msg_at: -1 })
...
router.post('/delete', authToken, async(req, res) => {
    ...
    try {
        ...
        if (msg.sender.toString() !== orgId.toString()) {
