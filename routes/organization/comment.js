...
router.post('/update', authToken, async (req, res) => {
    try {
        ...
        const comment = await Comment.findOne({ _id: commentId, organization_id: req.user._id, is_visible: true });
        if (!comment) {
...
router.post('/delete', authToken, async (req, res) => {
    try {
        ...
        const comment = await Comment.findOne({ _id: commentId, organization_id: req.user._id })
        if (!comment) {
            return res.status(404).send({ status: 'error', msg: 'Comment not found' })
        }
