...
router.post("/my_application", authToken, async (req, res) => {
    try {
        const application = await KIPApplication.findOne({ organization_id: req.user._id })
        if (!application) {
            return res.status(404).send({ status: 'error', msg: "Application not found" })
        }
...
router.post("/cancel", authToken, async (req, res) => {
    ...
    try {
        const application = await KIPApplication.findOne({ _id: applicationId, organization_id: req.user._id })
        if (!application) {
            return res.status(404).send({ status: 'error', msg: "Application not found" })
        }
