...
router.post('/initialize', async (req, res) => {
    try {
        ...
        return res.status(200).send({ status: 'ok', msg: 'success', data: response, reference })
    } catch (error) {
...
// View organization's payments
router.post('/view', authToken, async (req, res) => {
    try {
        const { _id } = req.user
        const payments = await Payment.find({ organization: _id }).lean()
